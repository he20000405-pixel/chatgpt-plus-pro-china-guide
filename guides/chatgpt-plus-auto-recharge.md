...
**这一步完成的标志：** 订单页面明确显示付款成功，并且你已经保存卡密和订单号。若只有银行待处理记录但页面没有生成卡密，请停止重复付款并联系页面客服核对订单。

### 3. 打开卡密核销页面

回到 <https://chonggrok.com/verify>，输入卡密并验证。核销域名应为 `chonggrok.com`，不要通过搜索广告、陌生短链或仿冒页面进入。

**这一步完成的标志：** 页面确认卡密有效，并进入账号资料步骤。若提示卡密无效、已使用或订单状态异常，请停止操作，保留卡密和订单号联系售后，不要再次购买。
...
**这一步完成的标志：** 页面接受本次升级所需信息并识别账号。若页面无法识别账号，先确认 ChatGPT 仍保持登录；仍无法识别时停止提交，不要向聊天窗口发送完整 session。
...
**停止条件：** 识别到的账号、登录方式或当前订阅与预期不一致时，不要点击确认。先退出错误账号并重新核对。

### 6. 等待系统自动完成升级

系统会根据已核销订单自动处理。处理中不要重复提交同一卡密，也不要同时在其他渠道为同一账号购买订阅，以免出现重复订单或状态判断困难。

**这一步完成的标志：** 核销页面显示任务完成或可查看处理结果。若页面仍显示处理中，请保留订单号等待当前任务结束，不要创建第二笔订单。
...
当账号、Plus 状态和到期时间三项都正确时，才算完成验收。若核销页显示完成但官方页面仍为 Free，请停止购买并进入“已付款仍显示 Free”专题。
